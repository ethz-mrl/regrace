...
class SemKITTI_demo(data.Dataset):
    ...
    def __getitem__(self, index):
        raw_data = np.fromfile(self.im_idx[index], dtype=np.float32).reshape(
            (-1, 4))
        if self.imageset == 'demo':
            ...
            annotated_data = np.fromfile(self.label_idx[index],
...
class SemKITTI_sk(data.Dataset):
    ...
    def __init__(self,
        ...
        for i_folder in split:
            self.im_idx += absoluteFilePaths('/'.join(
                [data_path, str(i_folder).zfill(2), 'velodyne']))
    ...
    def __getitem__(self, index):
        ...
        if self.imageset == 'test':
            ...
            annotated_data = np.fromfile(
                self.im_idx[index].replace('velodyne', 'labels')[:-3] +
                'label',
                dtype=np.uint32).reshape((-1, 1))
            annotated_data = annotated_data & 0xFFFF  # delete high 16 digits binary
            annotated_data = np.vectorize(
...
class SemKITTI_nusc(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        lidar_sd_token = self.nusc.get('sample',
                                       info['token'])['data']['LIDAR_TOP']
...
class SemKITTI_sk_multiscan(data.Dataset):
    ...
    def __init__(self,
                 ...
                 imageset='train',
                 return_ref=False,
                 label_mapping="semantic-kitti-multiscan.yaml"):
        self.return_ref = return_ref
        with open(label_mapping, 'r') as stream:
            semkittiyaml = yaml.safe_load(stream)
        self.learning_map = semkittiyaml['learning_map']
    ...
    def load_calib_poses(self):
        ...
        for seq in range(0, 22):
            ...
            # Read poses
            poses_f64 = self.parse_poses(join(seq_folder, 'poses.txt'),
                                         self.calibrations[-1])
    ...
    def fuse_multi_scan(self, points, pose0, pose):
        ...
        new_coords = np.sum(np.expand_dims(new_coords, 2) * pose0[:3, :3],
                            axis=1)
    ...
    def __getitem__(self, index):
        ...
        if number_idx - self.multiscan >= 0:
            ...
            for fuse_idx in range(self.multiscan):
                ...
                newpath2 = self.im_idx[index][:-10] + str(
                    number_idx - plus_idx).zfill(6) + self.im_idx[index][-4:]
                raw_data2 = np.fromfile(newpath2, dtype=np.float32).reshape(
                    (-1, 4))

                if self.imageset == 'test':
                    annotated_data2 = np.expand_dims(np.zeros_like(
                        raw_data2[:, 0], dtype=int),
                                                     axis=1)
                else:
                    annotated_data2 = np.fromfile(
                        newpath2.replace('velodyne', 'labels')[:-3] + 'label',
